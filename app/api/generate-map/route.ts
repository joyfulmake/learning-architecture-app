          ...
          nodes: {
            ...
            items: {
              ...
              properties: {
                ...
                connectionType: { type: "string", enum: ["code", "theory", "practice"] },
...
Write every field like you actually care whether the reader gets this right, not like an encyclopedia entry. Real motivation, real stakes, the actual reason a practitioner would lean forward, not a flat recitation of facts in the same register from node to node. If two nodes in this map would sound interchangeable with their subject matter swapped, that's the monotone failure mode, go back and put the specific charge back in.
...
- "connectionType": classify the dependency you just described in "connection" as exactly one of "code", "theory", or "practice". "code" means an actual implementation level dependency, you cannot build, run, or write the thing without the prerequisite already existing in the system. "theory" means a conceptual or mathematical foundation, you cannot correctly reason about, design, or predict the behavior of this node without understanding the prerequisite first, even before any code exists. "practice" means a real world, operational, or deployment dependency, one that only actually bites once this is used for real, under real conditions, load, or organizational constraints, not something that shows up in a clean theoretical treatment. Pick the one that's honestly true for this specific link, not whichever sounds more impressive. Phase 1 nodes (no dependsOn) should still classify their own foundational role this way, almost always "theory".
- "structure": what this concept is actually composed of and how those parts connect, grounded in a REAL implementation, not a generic abstract description. Name the actual components, modules, or data structures as they exist in a specific real, currently used codebase, open source project, production system, or published research architecture, one to two sentences. If you are not genuinely confident of a specific real implementation for this exact subtopic, describe the composition using the real terms practitioners actually use for it, and say plainly that you're describing the general pattern rather than one named system, instead of inventing a fake specific one. Distinct from "what", which defines the concept in the abstract; this is what it actually looks like built.
